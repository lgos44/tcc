...
Em \cite{leite2011servo} utiliza-se o controle de força com base em uma abordagem de controle cinemático. Considerando o problema em que o objetivo de controle é seguir uma força desejada variante no tempo $f_d(t)$ a partir da força de contato $f$ ao longo da superfície de restrição, medida com um sensor de força montado no punho do robô:
\begin{equation} \label{eq:problema_ctrl_forca}
f \rightarrow f_d(t), \qquad e_f = f_d(t) - f \rightarrow 0
\end{equation}
...
Derivando \eqref{eq:problema_ctrl_forca} e \eqref{eq:hooke_mat} em relação ao tempo