...
\end{equation}
onde utiliza-se a notação $s_i$ para $\sin \theta_i$ e $c_i$ para $\cos \theta_i$. No caso de $s_{ij}$ denota $\sin (\theta_i + \theta_j$.
...
\end{equation}
\subsection{Referencial Efetuador}
...
\bm{u} = \bm{J}_0^{-1}(\bm{q})  \bm{K}_p (\bm{x_d} - \bm{x})
\end{equation}
...
\section{Controle por Servo Visão}
...
Utilizando o sensor de força montado no efetuador final do manipulador, deseja-se fazer o controle da força aplicada sobre uma superfície. 
